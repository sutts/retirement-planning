...
export function calculateProjections(p) {
  ...
  for (let m = 0; m < 600; m++) {
    ...
    const netDraw = Math.max(0, monthlySpend - monthlySuperIncome);
    ...
    if (startCap > 0) {
      ...
      endCap = startCap - netDraw + netGain;
    ...
    if (m === (p.secondaryCapYear - 1) * 12 + 11) {
      // Injection happens at the END of the specified year (e.g. Y10 M12)
      injection = p.secondaryCapAmount * inflationMult;
      endCap += injection;
    }
    ...
    monthlyData.push({
      ...
      taxPaid,
      netGain,
      endCap: endCap - injection, // match previous logic where injection was added to currentBalance but endCap in row was before it?
      // Actually let's look at the old logic:
      // currentBalance += injection;
      // ...
      // endCap: endCap,
      // injection
      // Wait, old logic:
      // endCap = startCap - netDraw + netGain;
      // if (m === ...) { injection = ...; currentBalance += injection; }
      // monthlyData.push({ ..., endCap, injection });
      // So endCap in monthlyData was the balance BEFORE injection.
      injection
    });
  }
