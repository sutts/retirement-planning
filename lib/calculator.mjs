...
export function calculateProjections(p) {
  ...
  // Geometric compounding math patch & PIR Tax integration
  let effectiveTaxRate = p.taxRate;
  if (p.pirRate !== undefined && p.pirRate !== 'custom') {
    const pir = parseFloat(p.pirRate) / 100;
    // Balanced allocation formula: 50% shares (FDR method), 50% fixed interest (taxed on yield)
    // Taxable Yield = (50% * Gross Return) + (50% * 5.0% FDR rate)
    const taxableYield = (0.50 * p.grossReturnRate) + (0.50 * 0.05);
    effectiveTaxRate = Math.max(0, taxableYield * pir);
  }

  const netAnnualRate = Math.max(0, p.grossReturnRate - effectiveTaxRate);
  ...
  // Year 0 means "inject immediately" — add to starting balance before any month runs
  // so the capital earns returns from Month 1 onwards.
  let upfrontInjection = 0;
  if (p.secondaryCapYear === 0 && p.secondaryCapAmount > 0) {
    upfrontInjection = p.secondaryCapAmount; // no inflation adjustment — it's today's money
    currentBalance += upfrontInjection;
  }
  ...
  for (let m = 0; m < 600; m++) {
    ...
    let baseMonthlyRent = p.p3Rent || 0;
    ...
    const monthlyRent = baseMonthlyRent * inflationMult;
    const monthlySpend = (baseMonthlySpend + (yearlyExtra / 12)) * inflationMult + monthlyRent;
    ...
    let injection = 0;
    // Record the upfront injection on Month 1 (m=0) for display purposes
    if (m === 0 && upfrontInjection > 0) {
      injection = upfrontInjection;
    }
    // Year >= 1: inject at the end of the last month of the specified year
    if (p.secondaryCapYear >= 1 && m === (p.secondaryCapYear - 1) * 12 + 11) {
      // Use m+1 so end-of-year injections reflect the full elapsed year of
  ...
  for (let y = 0; y < 50; y++) {
    ...
    const rent = yearMonths.reduce((acc, row) => acc + row.rent, 0);
  ...
  return { yearlyData, monthlyData, yearsDepleted, effectiveTaxRate };
}
